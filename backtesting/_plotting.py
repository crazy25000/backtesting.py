...
return this.labels[index] || "";
    ...
    def _plot_superimposed_ohlc():
        ...
        if not resample_rule:
            warnings.warn(
                f"'Can't superimpose OHLC data with rule '{resample_rule}'" f"(index datetime resolution: '{time_resolution}'). Skipping.",
                stacklevel=4,
            )
            return
        ...
        for i, value in enumerate(indicators):
            ...
            for j, arr in enumerate(value, 1):
                ...
                if is_overlay:
                    ...
                    if not np.isnan(mean) and (abs(mean) < 0.1 or round(abs(mean), 1) == 0.5 or round(abs(mean), -1) in (50, 100, 200)):
                        fig.add_layout(
                            Span(
                                location=float(mean),
                                dimension='width',
